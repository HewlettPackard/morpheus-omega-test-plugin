 ...
 */
@Slf4j
class OmegaBackupProviderBackupTypeProvider extends AbstractBackupTypeProvider {

	BackupExecutionProvider executionProvider;
	BackupRestoreProvider restoreProvider;

	OmegaBackupProviderBackupTypeProvider(Plugin plugin, MorpheusContext morpheusContext) {
	...
	@Override
	String getCode() {
		return "omega-backupBackupTypeProvider"
	}
	...
	@Override
	String getName() {
		return "Omega Backup ProviderBackupTypeProvider"
	}
	 ...
	 */
	@Override
	OmegaBackupProviderBackupExecutionProvider getExecutionProvider() {
		if(!this.executionProvider) {
			this.executionProvider = new OmegaBackupProviderBackupExecutionProvider(getPlugin())
		}
	 ...
	 */
	@Override
	OmegaBackupProviderBackupRestoreProvider getRestoreProvider() {
		if(!this.restoreProvider) {
		this.restoreProvider = new OmegaBackupProviderBackupRestoreProvider(getPlugin())
		}
