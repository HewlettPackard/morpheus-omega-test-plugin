package com.morpheusdata.omega.backup
...
@Slf4j
class OmegaBackupProviderBackupProvider extends AbstractBackupProvider {

	BackupJobProvider backupJobProvider;

	OmegaBackupProviderBackupProvider(Plugin plugin, MorpheusContext morpheusContext) {
		super(plugin, morpheusContext)

		OmegaBackupProviderBackupTypeProvider backupTypeProvider = new OmegaBackupProviderBackupTypeProvider(plugin, morpheus)
	...
	@Override
	String getName() {
		return 'Omega Backup Provider Backup Provider'
	}
	...
	ServiceResponse refresh(BackupProviderModel backupProviderModel) {
		...
		def intervalMinutes = backupProviderModel.refreshIntervalMinutes ?: 'default'
		def nextRun = backupProviderModel.nextRunDate?.format("yyyy-MM-dd'T'HH:mm:ss.SSSZ") ?: 'not set'
		log.info("MORPH-6073 TEST: refresh() called at ${now} for provider '${backupProviderModel.name}' (id=${backupProviderModel.id}, refreshIntervalMinutes=${intervalMinutes}, nextRunDate=${nextRun})")
		backupProviderModel.statusMessage = "Last refresh: ${now} | interval: ${intervalMinutes}m"
