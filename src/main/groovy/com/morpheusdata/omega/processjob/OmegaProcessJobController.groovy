...
class OmegaProcessJobController implements PluginController {
	...
	def startSystemProcess(ViewModel<Map> model) {
		try {
			...
			ServiceResponse<ProcessModel> startResponse = morpheusContext.services.process.startProcess(
				system,
				ProcessStepType.forCode(stepTypeCode),
				user,
				description
			)
			ProcessModel process = startResponse?.data

			if (!startResponse?.success) {
				return errorResponse(startResponse?.msg ?: "Failed to start system process for system ${systemId}")
			}
